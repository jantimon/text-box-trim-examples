"use client";
import postcss, { Container, Declaration, PluginCreator, Root, Rule, ChildNode } from "postcss";
...
const getFirstFont = (node: Declaration) => {
  ...
  const font = fonts.split(",")[0].trim().replace(/([" '])/g, "");
  const fontMetricName = (Object.keys(metrics) as Array<keyof typeof metrics>).find((key) => key.toLowerCase() === font);
  if (!fontMetricName) {
    return;
  }
...
const getFontMetrics = (name: keyof typeof metrics, textEdges: Set<"leading" | "text" | "cap" | "ex" | "ideographic" | "ideographic-ink" | "alphabetic">) => {
  ...
  const lineGap = `--font-${fontName}-line-gap: ${metric.lineGap / metric.unitsPerEm}em;`;
  const ascent = `--font-${fontName}-ascent: ${metric.ascent / metric.unitsPerEm}em;`;
  if (textEdges.has("text")) {
    definitions.add(`--font-${fontName}-leading-trim-start-text: calc(var(--font-${fontName}-line-gap) / 2);`);
    definitions.add(`--font-${fontName}-leading-trim-end-text: calc(var(--font-${fontName}-line-gap) / 2);`);
  }
  if (textEdges.has("cap")) {
    ...
    definitions.add(`--font-${fontName}-cap-height: ${metric.capHeight / metric.unitsPerEm}em;`);
    definitions.add(lineGap);
    definitions.add(`--font-${fontName}-leading-trim-start-cap: calc((var(--font-${fontName}-cap-height) - var(--font-${fontName}-ascent) + var(--font-${fontName}-line-gap) / 2));`);
  }
  if (textEdges.has("ex")) {
    ...
    definitions.add(ascent);
    definitions.add(lineGap);
    definitions.add(`--font-${fontName}-x-height: ${metric.xHeight / metric.unitsPerEm}em;`);
    definitions.add(`--font-${fontName}-leading-trim-start-ex: calc((var(--font-${fontName}-x-height) - var(--font-${fontName}-ascent) + var(--font-${fontName}-line-gap) / 2));`);
  }
  if (textEdges.has("alphabetic")) {
    if ( !("descent" in metric)) {
      throw new Error(`alphabetic not supported for font "${name}"`);
    }
    definitions.add(`--font-${fontName}-descent: ${metric.descent / metric.unitsPerEm}em;`);
    definitions.add(lineGap);
    definitions.add(`--font-${fontName}-leading-trim-end-alphabetic: calc((var(--font-${fontName}-descent) - var(--font-${fontName}-line-gap) / 2));`);
  }
...
const postcssLeadingTrim: PluginCreator<pluginOptions> = (opts?: pluginOptions) => ({
    ...
    prepare: () => {
      ...
      const defaultFont: keyof typeof metrics | undefined = opts && "defaultFont" in opts ? opts.defaultFont : "timesNewRoman";
      return {
        OnceExit () {
          if (!root) {
            return;
      ...
      if (state.trim.size === 0 || (state.trim.size === 1 && state.trim.has("normal"))) {
        return;
      }
      // Skip if no text-over-edge or text-under-edge properties are used
      const overEdges = state.trim.has("start") || state.trim.has("both") ? Array.from(state.textOverEdge) : [];
      const underEdges = state.trim.has("end") || state.trim.has("both") ? Array.from(state.textUnderEdge) : [];
      ...
      state.rule.forEach((font, rule) => {
        ...
        if (textEdges.has("text")) {
          definitions.add(`--leading-trim-start-text: var(--font-${fontName}-leading-trim-start-text);`);
          definitions.add(`--leading-trim-end-text: var(--font-${fontName}-leading-trim-end-text);`);
        }
        if (textEdges.has("cap")) {
          definitions.add(`--leading-trim-start-cap: var(--font-${fontName}-leading-trim-start-cap);`);
        }
        if (textEdges.has("ex")) {
          definitions.add(`--leading-trim-start-ex: var(--font-${fontName}-leading-trim-start-ex);`);
        }
        if (textEdges.has("alphabetic")) {
          definitions.add(`--leading-trim-end-alphabetic: var(--font-${fontName}-leading-trim-end-alphabetic);`);
        }
